...
console.log(`navigate ${job.job_name} ${job.company}`);
await cdp.send("Page.navigate", { url: job.detail_url });
for (const sec of [6, 10, 15, 25]) {
  ...
  `);
  console.log(JSON.stringify({ afterSec: sec, ...result, descLen: result.desc.length, addressLen: result.address.length }, null, 2));
  if (result.desc.length > 20) break;
}
cdp.close();
